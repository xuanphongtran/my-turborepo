...
import { COOKIES_KEY, DEVELOPMENT, PAGE_QUERY_PARAMS } from '@repo/global-constants/common'
import type { ApiResponse } from '@repo/global-types/api.d'
import { useFetch } from './lib/utils/util-funcs'
import type { PageData } from '@repo/global-types/page.d'

export const config = {
  matcher: ['/((?!api|_next/static|favicon.ico|submit-payment|pagenotfound|404|robots|assets).*)']
}
...
export async function middleware(request: NextRequest) {
  const cloudflare = process.env.NODE_ENV === DEVELOPMENT ? 'CH' : (request.headers.get('cf-ipcountry') as string)
...
const getPathName = async (pathname: string, pageLevelRouter: string, lang: string, location: string) => {
  ...
  if (!pageLevelRouter) return { pathnameByLangLocation: `/${lang}-${location}}` }

  if (DEFAULT_EXCEPT_PAGE.includes(pageLevelRouter)) return { pathnameByLangLocation: pathname }
  ...
  if (pageSlugByLangLocation.data && pageSlugByLangLocation.data.length > 0 && pageLevelRouter) {
    ...
    } else return { pathnameByLangLocation: pathname, pageKeyword: pageLevelRouter }
  }
